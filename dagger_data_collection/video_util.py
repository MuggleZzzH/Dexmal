        ...
        if self.process:
            try:
                ...
                if self.process.returncode != 0:
                    ...
                    logger.error(f"FFmpeg错误: returncode={self.process.returncode}, stderr={stderr}")
                else:
...
def test_h264_video():
    ...
    try:
        logger.info("生成H264测试数据...")
        test_frames = []
        for i in range(n_frames):
            frame = np.zeros((height, width, 3), dtype=np.uint8)
            frame[:, :, 0] = (i * 255 // n_frames)
            frame[:, :, 1] = 128
            frame[:, :, 2] = (255 - i * 255 // n_frames)
            test_frames.append(frame)

        logger.info("写入H264视频...")
        writer = H264VideoWriter(test_file, width, height, fps, pixel_format="rgb24")
        for frame in test_frames:
            writer.write(frame)
        writer.release()
        logger.info(f"H264视频写入完成: {test_file}")

        logger.info("读取H264视频并验证...")
        reader = H264VideoReader(test_file, pixel_format="rgb24")
        read_frames = []
        ...
        if len(test_frames) != len(read_frames):
            logger.error(f"帧数不匹配: 原始 {len(test_frames)} vs 读取 {len(read_frames)}")
            return False

        logger.info("验证H264视频质量...")
        psnr_values = []
        for i, (original, read) in enumerate(zip(test_frames, read_frames)):
            if original.shape != read.shape:
                logger.error(f"帧 {i} 形状不匹配: {original.shape} vs {read.shape}")
                return False

            mse = np.mean((original.astype(float) - read.astype(float)) ** 2)
            if mse == 0:
                psnr = float("inf")
            else:
            ...
            # PSNR > 30dB 应该为高质量
            if psnr < 30:
                logger.warning(f"帧 {i} PSNR较低: {psnr:.2f} dB")

        avg_psnr = np.mean(psnr_values)
        logger.info(f"平均PSNR: {avg_psnr:.2f} dB")
        logger.info(f"PSNR范围: {np.min(psnr_values):.2f} - {np.max(psnr_values):.2f} dB")

        if avg_psnr >= 30:
            logger.info("H264视频测试通过！视频质量良好")
            os.remove(test_file)
            return True
        else:
            logger.error(f"H264视频质量较低: 平均PSNR {avg_psnr:.2f} dB < 30 dB")
            return False

    except Exception as e:
        logger.exception(f"H264视频测试失败: {e}")
...
def test_depth_video():
    ...
    try:
        logger.info("生成深度测试数据...")
        test_frames = []
        for i in range(n_frames):
            frame = np.zeros((height, width), dtype=np.uint16)
            for y in range(height):
                frame[y, :] = (y * 65535 // height + i * 100) % 65536
            test_frames.append(frame)
        ...
        for frame in test_frames:
            writer.write(frame)
        writer.release()
        logger.info(f"深度视频写入完成: {test_file}")
        ...
        for i, (original, read) in enumerate(zip(test_frames, read_frames)):
            ...
            if not np.array_equal(original, read):
                logger.error(f"帧 {i} 数据不匹配")
                diff = np.abs(original.astype(np.int32) - read.astype(np.int32))
                logger.error(f"最大差异: {np.max(diff)}")
                logger.error(f"差异像素数: {np.count_nonzero(diff)}")
                return False

        logger.info("深度视频测试通过！所有数据完全匹配")
        os.remove(test_file)
        return True

    except Exception as e:
        logger.exception(f"深度视频测试失败: {e}")
        if os.path.exists(test_file):
            os.remove(test_file)
        return False
...
def run_all_tests():
    ...
    # 测试 H264VideoWriter
    logger.info("\n" + "=" * 60)
    logger.info("测试 1/2: H264VideoWriter")
    logger.info("=" * 60)
    try:
        results["H264VideoWriter"] = test_h264_video()
    except Exception as e:
        logger.exception(f"H264VideoWriter 测试异常: {e}")
        results["H264VideoWriter"] = False

    # 测试 DepthVideoWriter
    logger.info("\n" + "=" * 60)
    logger.info("测试 2/2: DepthVideoWriter")
    logger.info("=" * 60)
    try:
        results["DepthVideoWriter"] = test_depth_video()
    except Exception as e:
        logger.exception(f"DepthVideoWriter 测试异常: {e}")
        results["DepthVideoWriter"] = False
